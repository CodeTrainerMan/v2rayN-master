...
namespace v2rayN.ViewModels
{
    public class MainWindowViewModel : ReactiveObject
    {
        ...
        public ReactiveCommand<Unit, Unit> AutoChangeCmd { get; }
        [Reactive]
        public bool BlReloadEnabled { get; set; }

        [Reactive]
        public bool AutoChangeFlag { get; set; }
        ...
        public MainWindowViewModel(ISnackbarMessageQueue snackbarMessageQueue, Action<EViewAction> updateView)
        {
            ...
            AutoChangeCmd = ReactiveCommand.Create(() =>
            {
                SetAutoChange();
        ...
        public void SetAutoChange()
        {
            ...
            autoThread = new Thread(new ParameterizedThreadStart(AutoChangeService));
            AutoChangeFlag = true;
            autoThread.IsBackground = true;
            autoThread.Start(this); 
            
           

        }
        private static void AutoChangeService(object mainWindowViewModel) {
            MainWindowViewModel mainWindowViewModel1 = mainWindowViewModel as MainWindowViewModel;
            List<ProfileItem> profileItems = LazyConfig.Instance.ProfileItems("");
            for (int i = 0; i < profileItems.Count; i++)
            {
                mainWindowViewModel1.SetDefaultServer(profileItems[i].indexId);
                /*SetDefaultServer(profileItems[i].indexId);*/
                if (i == (profileItems.Count - 1)) {
                    i = 0;
                }
                /* Thread.CurrentThread.Interrupt();*/           
                Thread.Sleep(15000);                
                if (mainWindowViewModel1.AutoChangeFlag==false) {
                    break;
                }
       ...
       /* private static void SetDefaultServer2(string indexId)
        {
            if (Utils.IsNullOrEmpty(indexId))
            {
                return;
